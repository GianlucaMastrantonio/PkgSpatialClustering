...
#  obj_data_mcmc.gp[:, k] = cholesky_mat.L*rand(Normal(0.0, 1.0), obj_data_mcmc.n_points)
...
#        mu_post[i] += (obj_data_mcmc.obs[i] - obj_data_mcmc.mu[k]) / obj_data_mcmc.tau2[k]
#      end
#    end
...
#  for k in 1:obj_mixture_mcmc.K[1]
#    MH_ratio = 0.0
#    obj_data_prop.rho[k] = rand(Normal(obj_data_prop.rho[k], sample(sd_vector, 1)[1]))
...
#    MH_ratio += -0.5 * obj_data_prop.log_det[k] - 0.5 * transpose(obj_data_prop.gp[:, k]) * obj_data_prop.inv_sigma_mat[k] * obj_data_prop.gp[:, k]
#    MH_ratio -= -0.5 * obj_data_mcmc.log_det[k] - 0.5 * transpose(obj_data_mcmc.gp[:, k]) * obj_data_mcmc.inv_sigma_mat[k] * obj_data_mcmc.gp[:, k]
