function spatial_cluster_v1(;
    ...
    if typeof(obj_data) == GpDataMarginalized_Vers9
        ...
        obj_data_mcmc = [GpDataMarginalized_Vers9(obs=obj_data.obs, gp_init=deepcopy(obj_data.gp), mu_init=deepcopy(obj_data.mu), sigma2_init=deepcopy(obj_data.sigma2), tau2_init=deepcopy(obj_data.tau2), rho_init=deepcopy(obj_data.rho), obj_mixture=obj_mixture_mcmc[it], obj_graph=obj_graph_mcmc[it], temperature= temperatures[it]) for it in 1:n_temps]

        obj_data_prop = [GpDataMarginalized_Vers9(obs=obj_data.obs, gp_init=deepcopy(obj_data.gp), mu_init=deepcopy(obj_data.mu), sigma2_init=deepcopy(obj_data.sigma2), tau2_init=deepcopy(obj_data.tau2), rho_init=deepcopy(obj_data.rho), obj_mixture=obj_mixture_mcmc[it], obj_graph=obj_graph_mcmc[it], temperature=temperatures[it]) for it in 1:n_temps]
    else
        obj_data_mcmc = [GpData_Vers9(obs=obj_data.obs, gp_init=deepcopy(obj_data.gp), mu_init=deepcopy(obj_data.mu), sigma2_init=deepcopy(obj_data.sigma2), tau2_init=deepcopy(obj_data.tau2), rho_init=deepcopy(obj_data.rho), obj_mixture=obj_mixture_mcmc[it], obj_graph=obj_graph_mcmc[it], temperature=temperatures[it]) for it in 1:n_temps]

        obj_data_prop = [GpData_Vers9(obs=obj_data.obs, gp_init=deepcopy(obj_data.gp), mu_init=deepcopy(obj_data.mu), sigma2_init=deepcopy(obj_data.sigma2), tau2_init=deepcopy(obj_data.tau2), rho_init=deepcopy(obj_data.rho), obj_mixture=obj_mixture_mcmc[it], obj_graph=obj_graph_mcmc[it], temperature=temperatures[it]) for it in 1:n_temps]
    end


    #obj_graph_mcmc::GraphCluter_Vers6 = GraphCluter_Vers6(graph=obj_graph.graph, weight_mat=deepcopy(obj_graph.weight_mat), coords=obj_graph.coords)
    #obj_graph_prop::GraphCluter_Vers6 = GraphCluter_Vers6(graph=obj_graph.graph, weight_mat=deepcopy(obj_graph.weight_mat), coords=obj_graph.coords)
    ...
    for iMCMC = 1:sample_to_save
        for jMCMC = 1:thin_burnin
            ...
            for it in 1:size(temperatures,1)
                sampling_mcmc(iterations, obj_graph_mcmc[it], obj_graph_prop[it], obj_mixture_mcmc[it], obj_mixture_prop[it], obj_data_mcmc[it], obj_data_prop[it], obj_prior, temperatures[which_data_to_temp[it]])
            end
