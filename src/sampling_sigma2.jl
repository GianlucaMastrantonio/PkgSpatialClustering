...
function sampling_sigma2_cluster(iterations::Int64,
  ...
  for k in 1:obj_mixture_mcmc.K[1]
    obj_data_mcmc.sigma_mat[k].data .= obj_data_mcmc.sigma_mat[k].data ./ obj_data_mcmc.sigma2[k]
    obj_data_mcmc.inv_sigma_mat[k].data .= obj_data_mcmc.inv_sigma_mat[k].data .* obj_data_mcmc.sigma2[k]

    obj_data_mcmc.log_det[k] = obj_data_mcmc.log_det[k] - obj_data_mcmc.n_points *log(obj_data_mcmc.sigma2[k]) 
    
    a_p = params(obj_prior.sigma2)[1] + 0.5 * obj_data_mcmc.n_points
    b_p = params(obj_prior.sigma2)[2] + 0.5 * transpose(obj_data_mcmc.gp[:, k] .- obj_data_mcmc.mu[k]) * obj_data_mcmc.inv_sigma_mat[k] * (obj_data_mcmc.gp[:, k] .- obj_data_mcmc.mu[k])
    ...
    if iii == 10000
      ...
      if (app > params(obj_prior.sigma2)[3] && (app < params(obj_prior.sigma2)[4]))
        ...
        MH_ratio += -0.5 * obj_data_mcmc.n_points * log(app) + 0.5 * obj_data_mcmc.n_points * log(obj_data_mcmc.sigma2[k])
        MH_ratio += (-0.5 / app + 0.5 / obj_data_mcmc.sigma2[k]) * transpose(obj_data_mcmc.gp[:, k] .- obj_data_mcmc.mu[k]) * obj_data_mcmc.inv_sigma_mat[k] * (obj_data_mcmc.gp[:, k] .- obj_data_mcmc.mu[k])
