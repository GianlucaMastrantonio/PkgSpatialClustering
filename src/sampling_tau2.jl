function sampling_tau2(iterations::Int64,
  ...
  sampling_tau2_cluster(iterations, obj_graph_mcmc, obj_graph_prop, obj_mixture_mcmc, obj_mixture_prop, obj_data_mcmc, obj_data_prop, obj_prior)
...
function sampling_tau2_cluster(iterations::Int64,
  ...
  for iobs in 1:obj_data_mcmc.n_points
    ...
    a_p[k] += 0.5
    b_p[k] += 0.5 * (obj_data_mcmc.obs[iobs] -  obj_data_mcmc.gp[iobs, k])^2.0
    ...
    if iii == 10000
      ...
      if (app > params(obj_prior.tau2)[3] && (app < params(obj_prior.tau2)[4]))
        ...
        for iobs in 1:obj_data_mcmc.n_points
          ...
          if k == k_app
            MH_ratio += logpdf(Normal(obj_data_mcmc.gp[iobs, k], app^0.5), obj_data_mcmc.obs[iobs]) - logpdf(Normal(obj_data_mcmc.gp[iobs, k], obj_data_prop.tau2[k]^0.5), obj_data_mcmc.obs[iobs])
          end
        end
