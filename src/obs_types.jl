...
function update_param_cluster_divided(obj_data::GpDataMarginalized_Vers9, obj_mixture::TestMixture_V5, k::Int64)
    ...
    return -0.5 * obj_data.log_det[k] - 0.5 * obj_data.n_which_obs[k] * log(2.0 * pi), -0.5 * transpose(obj_data.obs[w_obs] .- obj_data.mu[k]) * obj_data.inv_sigma_mat[k][index, index] * (obj_data.obs[w_obs] .- obj_data.mu[k])
